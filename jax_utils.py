"""
    Utility functions for JAX copied from https://github.com/awf/functional-transformer.
"""
...
def print_param_names(params: dict, prefix: str = "", max_depth: int = 100):
    for k in params.keys():
        print(prefix + k)
        if max_depth > 0 and params[k] is not None and isinstance(params[k], dict):
            print_param_names(
                params[k], prefix=prefix + f"{k}.", max_depth=max_depth - 1
            )
...
class ParamsDict(types.SimpleNamespace):
    ...
    def tree_flatten(self):
        ...
        return jax.tree_flatten(self.__dict__, lambda a: a is not self.__dict__)
